...
export const COLORS = {
  ...
  none: [160, 190, 225],
  gap: [205, 240, 255],         // electrical coupling
...
// motor neuron in the tail near 1e-5, a faint smell's echo lower still.
// Brightness follows its logarithm, so a signal stays visible as it spreads and
// fades hop by hop.
...
function sprite(c) {
  ...
  const hot = c.map((v) => Math.round(v + 0.45 * (255 - v)));
  r.addColorStop(0, rgba(hot, 0.95));
  r.addColorStop(0.12, rgba(c, 0.85));
  r.addColorStop(0.36, rgba(c, 0.22));
...
export class Renderer {
  ...
  draw(life, view) {
    ...
    const mid = this.at(f, 0.3, 0), target = this.baseScale() * this.zoom;
    ...
    this.nervous(g, f, T, life, view, { W, H, zs, head: 0.45, minSprite: 3.2, lw: Math.min(1.4, Math.max(0.5, S / 600)), lines: 700, pulses: 520, glow: 1, ringLines: false, edges: this.edges });
  ...
  // the whole nervous system: scaffold, cords, live connections, vesicles, lessons, somata
  nervous(g, f, T, life, view, o) {
    ...
    // the wiring the brain is allowed to use, faint: synapses and gap junctions in the
    ...
    g.strokeStyle = "rgba(120,190,255,0.05)"; g.lineWidth = 0.6 * o.lw; g.stroke(wire);

    // the nerve cords, lit by the cells along them
    const NB = 72, bin = new Float32Array(NB);
    for (const c of cells) { const b = Math.min(NB - 1, Math.floor(c.s * NB)); bin[b] = Math.max(bin[b], lum(act[c.i])); }
    for (let k = 1; k < NB; k++) bin[k] = Math.max(bin[k], 0.82 * bin[k - 1]);
    for (let k = NB - 2; k >= 0; k--) bin[k] = Math.max(bin[k], 0.82 * bin[k + 1]);
    g.globalCompositeOperation = "lighter"; g.lineCap = "round";
    for (const [side, gain] of [[VENTRAL, 1], [DORSAL, 0.55]]) {
      for (let k = 2; k < NB - 1; k++) {
        const a = T(...this.at(f, k / NB, side)), b = T(...this.at(f, (k + 1) / NB, side)), x = bin[k] * gain;
        g.strokeStyle = rgba(COLORS.body, 0.07 + 0.45 * x * x); g.lineWidth = (0.8 + 2.6 * x) * o.lw;
    ...
    for (const e of o.edges) {
      ...
      const L = lum(x * 10); if (L > 0.15) live.push([L, e]);
    }
    ...
    for (const [key, p] of paths) {
      ...
      g.strokeStyle = rgba(COLORS[fam], (fam === "gap" ? 0.6 : 1) * o.glow * (0.04 + 0.3 * x * x)); g.lineWidth = (0.4 + 1.0 * x) * o.lw;
      g.stroke(p);
    }
    for (const c of cells) if (c.dorsal) {       // commissures carry their motor neuron's activity
      const L = lum(act[c.i]); if (L < 0.15) continue;
      const p = new Path2D(); this.commissure(p, f, T, c);
      g.strokeStyle = rgba(COLORS[c.family], 0.05 + 0.4 * L * L); g.lineWidth = (0.6 + 1.2 * L) * o.lw; g.stroke(p);
    }
    ...
    for (let n = 0, drawn = 0; n < live.length && drawn < o.pulses; n++) {
      const [L, e] = live[n]; if (e.gap || L < 0.22) continue;
      ...
      const sz = Math.max(2.5, (3 + 11 * L) * o.zs);
      g.globalAlpha = Math.min(1, 0.1 + 0.9 * L * L); g.drawImage(this.sprites[e.family], q[0] - sz / 2, q[1] - sz / 2, sz, sz);
      drawn++;
    }
    g.globalAlpha = 1;
    ...
    if (Ls && Ls.applied) {
      ...
      if (fade > 0) {
        ...
        for (const [d, e] of Ls.top) {          // the connections that changed most, lit along their paths
          ...
          g.strokeStyle = rgba(COLORS.learning, 0.85 * x * fade * reach); g.lineWidth = (0.8 + 1.8 * x) * o.lw; g.stroke(p);
        }
        for (const c of cells) {                 // each cell's share of the correction, arriving hop by hop
          ...
          g.strokeStyle = rgba(COLORS.learning, 0.9 * a); g.lineWidth = 1.4;
          g.beginPath(); g.arc(x, y, r, 0, Math.PI * 2); g.stroke();
          const sz = (10 + 30 * cr) * Math.max(0.55, o.zs);
          g.globalAlpha = Math.min(1, 0.8 * a); g.drawImage(this.sprites.learning, x - sz / 2, y - sz / 2, sz, sz); g.globalAlpha = 1;
        }
      }
    }

    // the neurons, every one, brightest on top
    g.globalCompositeOperation = "screen";
    for (const c of cells) {
      const [x, y] = pos[c.i]; if (x < -30 || y < -30 || x > o.W + 30 || y > o.H + 30) continue;
      const L = lum(act[c.i]), z = o.zs * (c.s < 0.12 ? o.head : 1);
      const sz = Math.max(o.minSprite, (5 + 22 * L) * z);
      g.globalAlpha = 0.18 + 0.72 * L; g.drawImage(this.sprites[c.family], x - sz / 2, y - sz / 2, sz, sz);
      g.globalAlpha = 1;
      g.fillStyle = `rgba(240,250,255,${0.2 + 0.7 * L * L})`;
      g.beginPath(); g.arc(x, y, Math.max(0.55, (0.8 + 1.0 * L) * z), 0, Math.PI * 2); g.fill();
    }
    g.globalCompositeOperation = "source-over";
  }
  ...
  body(g, f, T, view, S) {
    ...
    lg.addColorStop(0, "rgba(60,130,200,0.20)"); lg.addColorStop(0.5, "rgba(40,95,160,0.13)"); lg.addColorStop(1, "rgba(40,95,160,0.07)");
    g.fillStyle = lg; g.fill();
    // cuticle rings
    g.strokeStyle = "rgba(150,210,255,0.06)"; g.lineWidth = 1;
    ...
    g.strokeStyle = "rgba(170,225,255,0.18)"; g.lineWidth = 1;
    const bulb = (s, w) => {
    ...
    let head = 0, n = 0;
    for (const c of this.cells) if (c.s < 0.1) { head += lum(view.activity[c.i]); n++; }
    head /= Math.max(1, n);
    const ring = T(...this.at(f, 0.045, 0)), edge = T(...this.at(f, 0.045, 0.95));
    const rr = Math.hypot(edge[0] - ring[0], edge[1] - ring[1]);
    g.save(); g.globalCompositeOperation = "lighter";
    g.shadowColor = "rgba(127,231,255,0.9)"; g.shadowBlur = 8 + 30 * head;
    g.strokeStyle = `rgba(160,235,255,${0.25 + 0.6 * head})`; g.lineWidth = 1.2 + 2.4 * head;
    ...
    outline(); g.strokeStyle = "rgba(150,225,255,0.5)"; g.lineWidth = 1.3; g.stroke();
    g.restore();
  }
...
class BrainView {
  ...
  draw(life, view) {
    ...
    R.nervous(g, this.f, T, life, view, { W, H, zs: 0.6, head: 1, minSprite: 4, lw: 0.9, lines: 650, pulses: 320, glow: 0.75, ringLines: true, edges: this.edges });
    this.pos = R.pos;
    ...
    g.strokeStyle = "rgba(169,188,211,0.55)"; g.lineWidth = 1;
    ...
    } else this.say("Brightness is activity on a log scale, 10<sup>−6</sup> to 1. Point at a neuron to name it.");
  }
