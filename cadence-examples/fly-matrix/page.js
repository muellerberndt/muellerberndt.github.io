...
worker.onerror = (e) => { life.log(`the brain's worker failed: ${e.message || e}`); S.pending = false; S.pendingSince = 0; };
worker.onmessageerror = () => { S.pending = false; S.pendingSince = 0; };
...
function queueReward() { // the outcome of the open search goes to the worker before its next step
  ...
  if (r.fresh) { worker.postMessage({ type: "decide", stimuli: life.senses(null, 0), steps: 4, u: 0, blame: true }); S.pending = true; S.pendingSince = performance.now(); }
...
function togglePause() { S.paused = !S.paused; document.body.classList.toggle("paused", S.paused); life.log(S.paused ? "paused" : "resumed"); }
...
function fillCard() {
  const m = payloadInfo;
  $("card-brain").innerHTML = `<b>The brain.</b> The nervous system of an adult female <i>Drosophila melanogaster</i>, brain and nerve cord wired as measured (BANC release 888: ${whole.toLocaleString()} neurons, 1,877,099 synapse classes at five or more synapses, the Kenyon-cell-to-MBON seam at every count), as one Cadence brain with the library's graded rate model at one global gain selected on two physiology facts, and the antennal lobe's local neurons at a gain of their own selected on the Kenyon cell code (sparse and specific), because at the global gain their mutual excitation ignites the lobe and every odour makes the same code. The page draws every neuron at its soma position and settles the ${m.n.toLocaleString()}-neuron sub-net recruited from the flight, looming, odour, taste and mushroom-body populations, one step per ${BRAIN_MS} ms of simulated time; the sub-net's settled populations agree with the whole brain's under the page's stimuli to a readout deviation below 1e-3 (receipts/subnet_closure.json).`;
  ...
  $("card-claim").innerHTML = `<b>The lessons.</b> A hungry fly that notices a smell turns toward it by instinct and hovers over the fruit; there, once per search, the mushroom body decides whether to land on it or to leave it: the choice is a softmax over two output neurons of declared valence, MBON11 (γ1pedc>α/β, GABAergic, approach) and MBON05 (γ4>γ1γ2, glutamatergic, avoidance), by the transmitter rule of Aso et al. 2014. The turn toward or away from the smell is supplied, as the worm's undulation was. Sugar on the fruit it lands on is a reward of one, an empty fruit nothing when the fly leaves it, a blow from the hand while it sits there minus one; the library's actor-critic (eligibility traces on the nudged contrast, dopamine as the temporal-difference error, a critic on the Kenyon cells) moves the ${(S.lessonStats && S.lessonStats.plastic) || "Kenyon-cell-to-MBON"} synapses onto the MBONs, the site of the animal's olfactory memory. The lesson starts naive, every plastic class at the same weight, because a specimen's synapse counts at its memory site are that specimen's memories; the two output cells are calibrated over the situations the fly decides in, the approach cell to 0.6 and the avoidance cell to 0.4 (the naive fly's attraction to food smells), because a connectome carries no operating point and a cell on its rail has no slope for a nudge. The same rule and wiring run the receipted experiment (tools/learn_odour.py) against shuffled, frozen and MLP controls; the page's constants are the ones its source declares. Gate 2: 7 of 13 held-out physiology facts about the wing steering circuit pass on the measured wiring against 3, 1 and 1 on shuffled wirings. Gate 3: 8 of 14 instinct facts against 2 on each shuffled wiring. Gate 4: the T-maze reverses in both directions on the library's actor-critic (tools/tmaze.py), and not on a shuffled wiring.`;
...
function frame(now) {
  ...
  if (S.freeze) { life.clock += dtWall * S.speed; }
    ...
    if (S.pending && S.pendingSince && now - S.pendingSince > 5000) { life.log("the brain did not answer in 5 s; asking again"); S.pending = false; }  // the watchdog: a lost reply never stalls the page
    if (!S.pending && (S.brainDue >= BRAIN_MS || life.wantDecision)) {
      ...
      S.brainDue -= steps * BRAIN_MS; S.pending = true; S.pendingSince = now;
